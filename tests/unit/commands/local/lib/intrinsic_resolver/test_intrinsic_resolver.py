...
class TestIntrinsicFnSelectResolver(TestCase):
    ...
    @parameterized.expand([
        ("Fn::Select should fail if the index is out of bounds: {}".format(number), number)
        for number in
        [-2, 7]
    ])
...
class TestIntrinsicFnGetAttResolver(TestCase):
    ...
    @parameterized.expand([
        ("Fn::GetAtt should fail if it doesn't have exactly 2 arguments: {}"
         .format(item), item)
        for item in
...
class TestIntrinsicFnNotResolver(TestCase):
    ...
    @parameterized.expand([
        ...
        for item in
        [[True] * i for i in [0, 2, 3, 4, 5, 6, 7, 8, 9, 10]]
    ])
